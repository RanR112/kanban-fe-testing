...
import "../../../sass/User/UserLead/DetailReqLead/DetailReqLead.css";
...
export default function DetailReqLead() {
  const { id } = useParams();
  ...
  return (
    <div className="container-detail-userlead">
      <h2 className="head-detail-userlead">DETAILS REQUEST KANBAN</h2>
      <div className="grid-detail-userlead">
        <div className="production-date-detail-userlead">
          <label>Production Date</label>
          <input type="date" value={kanban.tgl_produksi} readOnly />
        </div>
        <div className="parts-number-detail-userlead">
          <label>Part Number</label>
          <input type="text" value={kanban.parts_number} readOnly />
        </div>
        <div className="request-name-detail-userlead">
          <label>Requesting Name</label>
          <input type="text" value={kanban.nama_requester} readOnly />
        </div>
        <div className="classification-detail-userlead">
          <label>Classification</label>
          <select value={kanban.klasifikasi} disabled>
            <option>{kanban.klasifikasi}</option>
          </select>
        </div>
        <div className="box-detail-userlead">
          <label>Box</label>
          <input type="text" value={kanban.box} readOnly />
        </div>
        <div className="process-detail-userlead">
          <label>Process</label>
          <input type="text" value={process} readOnly />
        </div>
        <div className="location-detail-userlead">
          <label>Location</label>
          <input type="text" value={kanban.lokasi} readOnly />
        </div>
        <div className="information-detail-userlead">
          <label>Information</label>
          <textarea value={kanban.keterangan} readOnly />
        </div>
      </div>

      <div className="statusContainer-detail-userlead">
        {Object.values(filteredPersetujuan).map((dept, index) => (
          <div key={index} className="card-detail-userlead">
            <div className="cardHeader-detail-userlead">
              <span>{`${
                DEPARTMENT_MAP[dept.department.name]
              } Department`}</span>
            </div>

            <div className="row-detail-userlead header-detail-userlead">
            ...
            {dept.roles.map((role, idx) => (
              <div key={idx} className="row-detail-userlead">
                <span
                  className={`role-detail-userlead ${
                ...
                >
                  <img
                    ...
                    alt=""
                    className="approval-icon-detail-userlead"
                  />
                  {role.role}
                </span>
                <span>
      ...
      <div className="button-back-detail-userlead">
        <button
          onClick={() => navigate(-1)}
          className="backButton-detail-userlead"
        >
          Back
        </button>
      </div>
